	...
	<div class="wrap">
		...
		<div class="sub">
			<div class="size">
				...
				<div class="bbs">
					<table class="list">
						<!-- replyVO는 VO에서 getter에서 get을뺀값을 쓰는것이기 때문에 소문자로 작성 -->
						<p>
							<span><strong>총 ${replyVO.totCount}개</strong> |
								${replyVO.reqPage}/${replyVO.totPage }페이지</span>
						</p>
						...
						<tbody>
							...
							<c:forEach var="vo" items="${list}" varStatus="status">
								<tr>
									<td>${(replyVO.totCount-status.index)-((replyVO.reqPage-1)*replyVO.pageRow)}</td>  <!-- 실제 게시판 번호가 아니라 순서의 역순으로 표시한것뿐(삭제된 게시글 영향 x) -->
									<td class="txt_l">
										<a href="detail.do?no=${vo.no}&reqPage=${replyVO.reqPage }&stype=${replyVO.stype}&sval=${replyVO.sval}&orderby=${replyVO.orderby}&direct=${replyVO.direct}">
					...
					<div class="pagenate clear">
						<ul class='paging'>
							<c:if test="${replyVO.startPage > replyVO.pageRange}">
								<li><a
									href="index.do?reqPage=${replyVO.startPage-1}&stype=${param.stype}&sval=${param.sval}"><</a>
								</li>
							</c:if>
									<!-- 동작하던 전 페이지로 가려면 파라미터를 물고 다녀야함 예를들어 상세 페이지에서 목록으로 갔을경우 보던페이지 출력시>  -->
									<!-- 즉 1보다 작으면 안되고, 이전을 누르면 시작페이지에서 1뺀 곧으로 이동 /for문안에 있으면 x -->

							<c:forEach var="rp" begin="${replyVO.startPage}"
								end="${replyVO.endPage}">
								<li><a
									href='index.do?reqPage=${rp}&stype=${param.stype}&sval=${param.sval}&orderby=${param.orderby}&direct=${param.direct}'
									<c:if test="${rp==replyVO.reqPage }">class='current'</c:if>>${rp}</a></li>
							</c:forEach>

							<c:if test="${replyVO.totPage > replyVO.endPage}">
								<li><a
									href="index.do?reqPage=${replyVO.endPage+1}&stype=${param.stype}&sval=${param.sval}&orderby=${param.orderby}&direct=${param.direct}">></a>
								</li>
								<!-- for문안에 있으면 x -->
							</c:if>
						</ul>
						...
						<form method="get" name="searchForm" id="searchForm" action="">
							<span class="srchSelect">
							 ...
							 <select id="stype" name="stype"
								...
								name="sval" value="${replyVO.sval}" title="검색어 입력"> <input
