	...
	<div class="wrap">
		...
		<div class="sub">
			<div class="size">
				...
				<div class="bbs">
					<table class="list">
						<!-- newsVO는 VO에서 getter에서 get을뺀값을 쓰는것이기 때문에 소문자로 작성 -->
						<p>
							<span><strong>총 ${newsVO.totCount}개</strong> |
								${newsVO.reqPage}/${newsVO.totPage }페이지</span>
						</p>
						...
						<tbody>
							...
							<c:forEach var="vo" items="${list}">
								<tr>
									<td>${vo.no }</td>
									<td class="txt_l"><a href="detail.do?no=${vo.no}&reqPage=${newsVO.reqPage }&stype=${newsVO.stype}&sval=${newsVO.sval}&orderby=${newsVO.orderby}&direct=${newsVO.direct}">
											${vo.title}</a></td>
					...
					<div class="pagenate clear">
						<ul class='paging'>
							<c:if test="${newsVO.startPage > newsVO.pageRange}">
								<li><a
									href="index.do?reqPage=${newsVO.startPage-1}&stype=${param.stype}&sval=${param.sval}"><</a>
								</li>
							</c:if>
									<!-- 동작하던 전 페이지로 가려면 파라미터를 물고 다녀야함 예를들어 상세 페이지에서 목록으로 갔을경우 보던페이지 출력시>  -->
									<!-- 즉 1보다 작으면 안되고, 이전을 누르면 시작페이지에서 1뺀 곧으로 이동 /for문안에 있으면 x -->

							<c:forEach var="rp" begin="${newsVO.startPage}"
								end="${newsVO.endPage}">
								<li><a
									href='index.do?reqPage=${rp}&stype=${param.stype}&sval=${param.sval}&orderby=${param.orderby}&direct=${param.direct}'
									<c:if test="${rp==newsVO.reqPage }">class='current'</c:if>>${rp}</a></li>
							</c:forEach>

							<c:if test="${newsVO.totPage > newsVO.endPage}">
								<li><a
									href="index.do?reqPage=${newsVO.endPage+1}&stype=${param.stype}&sval=${param.sval}&orderby=${param.orderby}&direct=${param.direct}">></a>
								</li>
								<!-- for문안에 있으면 x -->
							</c:if>
						</ul>
						...
						<form method="get" name="searchForm" id="searchForm" action="">
							<span class="srchSelect">
							 ...
							 <select id="stype" name="stype"
								...
								name="sval" value="${newsVO.sval}" title="검색어 입력"> <input
